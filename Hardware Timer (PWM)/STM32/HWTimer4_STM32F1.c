/***************************************************************************//**
 * @brief Hardware Timer 4 Implementation (STM32F1)
 * 
 * @file HWTimer4_STM32F1.c
 * 
...
HWTimerInterface HWTimer4_FunctionTable = {
    .HWTimer_GetPrescaleOptions = HWTimer4_STM32_GetPrescaleOptions,
    .HWTimer_ComputePeriod = (void (*)(void *, uint32_t, uint32_t, uint16_t *))HWTimer4_STM32_ComputePeriod,
    .HWTimer_Init = (void (*)(void *))HWTimer4_STM32_Init,
    .HWTimer_GetSize = HWTimer4_STM32_GetSize,
    .HWTimer_Start = HWTimer4_STM32_Start,
    .HWTimer_Stop = HWTimer4_STM32_Stop,
    .HWTimer_Reset = HWTimer4_STM32_Reset,
    .HWTimer_IsRunning = HWTimer4_STM32_IsRunning,
    .HWTimer_SetCount = HWTimer4_STM32_SetCount,
    .HWTimer_GetCount = HWTimer4_STM32_GetCount,
    .HWTimer_AddToCount = HWTimer4_STM32_AddToCount,
    .HWTimer_GetNumCompareChannels = HWTimer4_STM32_GetNumCompareChannels,
    .HWTimer_SetCompare16Bit = HWTimer4_STM32_SetCompare16Bit,
    .HWTimer_GetCompare16Bit = HWTimer4_STM32_GetCompare16Bit,
    .HWTimer_SetComparePercent = HWTimer4_STM32_SetComparePercent,
    .HWTimer_GetComparePercent = HWTimer4_STM32_GetComparePercent,
    .HWTimer_EnableComparePWM = HWTimer4_STM32_EnableComparePWM,
    .HWTimer_DisableComparePWM = HWTimer4_STM32_DisableComparePWM,
    .HWTimer_GetOverflow = HWTimer4_STM32_GetOverflow,
    .HWTimer_ClearOverflowFlag = HWTimer4_STM32_ClearOverflowFlag,
    .HWTimer_GetCompareMatch = HWTimer4_STM32_GetCompareMatch,
    .HWTimer_ClearCompareMatchFlag = HWTimer4_STM32_ClearCompareMatchFlag,
    .HWTimer_OverflowEvent = HWTimer4_STM32_OverflowEvent,
    .HWTimer_CompareMatchEvent = HWTimer4_STM32_CompareMatchEvent,
    .HWTimer_SetOverflowCallback = HWTimer4_STM32_SetOverflowCallback,
    .HWTimer_SetCompareMatchCallback = HWTimer4_STM32_SetCompareMatchCallback,
};
...
HWTimerPrescaleOptions HWTimer4_STM32_GetPrescaleOptions(void)
{
...
void HWTimer4_STM32_ComputePeriod(HWTimerInitType_STM32 *retParams, 
...
void HWTimer4_STM32_Init(HWTimerInitType_STM32 *params)
{
...
HWTimerSize HWTimer4_STM32_GetSize(void)
{
    return HWTIM_16_BIT;
}
...
void HWTimer4_STM32_Start(void)
{
...
void HWTimer4_STM32_Reset(void)
{
    TIMx->CNT = 0;
}
...
bool HWTimer4_STM32_IsRunning(void)
{
...
void HWTimer4_STM32_SetCount(uint16_t count)
{
    TIMx->CNT = count;
}
...
uint16_t HWTimer4_STM32_GetCount(void)
{
    return TIMx->CNT;
}
...
void HWTimer4_STM32_AddToCount(uint16_t addToCount)
{
    TIMx->CNT += addToCount;
}
...
uint8_t HWTimer4_STM32_GetNumCompareChannels(void)
{
...
void HWTimer4_STM32_SetCompare16Bit(uint8_t compChan, uint16_t compValue)
{
...
uint16_t HWTimer4_STM32_GetCompare16Bit(uint8_t compChan)
{
    if(compChan >= HW_TIM_NUM_COMP_CHANNELS)
        return 0;
...
void HWTimer4_STM32_SetComparePercent(uint8_t compChan, uint8_t percent)
{
    if(compChan >= HW_TIM_NUM_COMP_CHANNELS)
        return;
...
uint8_t HWTimer4_STM32_GetComparePercent(uint8_t compChan)
{
    if(compChan >= HW_TIM_NUM_COMP_CHANNELS)
        return 0;
...
void HWTimer4_STM32_EnableComparePWM(uint8_t compChan)
{
    if(compChan < HW_TIM_NUM_COMP_CHANNELS)
    {
...
void HWTimer4_STM32_DisableComparePWM(uint8_t compChan)
{
    if(compChan < HW_TIM_NUM_COMP_CHANNELS)
    {
        /* Select either the upper or lower OCxM bits (0 or 1) */
...
bool HWTimer4_STM32_GetOverflow(void)
{
...
void HWTimer4_STM32_ClearOverflowFlag(void)
{
    TIMx->SR &= ~TIM_SR_UIF;
}
...
bool HWTimer4_STM32_GetCompareMatch(uint8_t compChan)
{
    if(compChan < 4)
    {
...
void HWTimer4_STM32_ClearCompareMatchFlag(uint8_t compChan)
{
    if(compChan < 4)
    {
...
void HWTimer4_STM32_OverflowEvent(void)
{
    TIMx->SR &= ~TIM_SR_UIF;
...
void HWTimer4_STM32_CompareMatchEvent(void)
{
...
void HWTimer4_STM32_SetOverflowCallback(void (*Function)(void))
...
void HWTimer4_STM32_SetCompareMatchCallback(void (*Function)(uint8_t compChan))
